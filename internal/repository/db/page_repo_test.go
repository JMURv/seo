...
func TestRepository_ListPages(t *testing.T) {
	...
	const q = `SELECT slug, title, href, created_at, updated_at FROM page`

	t.Run("Success", func(t *testing.T) {
		rows := sqlmock.NewRows([]string{"slug", "title", "href", "created_at", "updated_at"}).
			AddRow(expectedPages[0].Slug, expectedPages[0].Title, expectedPages[0].Href, expectedPages[0].CreatedAt, expectedPages[0].UpdatedAt).
			AddRow(expectedPages[1].Slug, expectedPages[1].Title, expectedPages[1].Href, expectedPages[1].CreatedAt, expectedPages[1].UpdatedAt)
...
func TestRepository_GetPage(t *testing.T) {
	...
	q := regexp.QuoteMeta(`SELECT slug, title, href, created_at, updated_at FROM page WHERE slug = $1`)
...
func TestRepository_CreatePage(t *testing.T) {
	...
	const q = `INSERT INTO page (slug, title, href) 
		 VALUES ($1, $2, $3)
		 RETURNING slug`
